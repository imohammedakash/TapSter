...
const Wrapper = React.memo(({ data }) => {
    return (
        <div className={`h-[17rem] w-[17rem] bg-white p-1 relative flex items-center justify-center shadow border rounded hover:shadow-xl hover:scale-105 transition-all cursor-pointer after:absolute after:content-[''] after:rounded-[inherit]  after:w-full after:h-full after:z-[2] after:bg-gradient-to-br after:from-transparent after:to-black`}>
            <img className="h-full w-full object-cover rounded-[inherit]" src={data.media.imageUrl} alt={data?.categoryName ?? `category-name`} />
            <span className='absolute z-[4] text-white capitalize text-2xl '>{data.categoryName}</span>
        </div>
    )
})
